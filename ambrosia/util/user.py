...
DB_FILE = "/var/www/ambrosia/ambrosia/data/health.db"
...
def register(username, password):
    ...
    for row in data:
        if username == row[1]:
            return False
    params = (username, passlib.hash.sha256_crypt.hash(password))
...
def authenticate(username, password):
    ...
    data = c.execute("SELECT * FROM users")
    for row in data:
        if row[1] == username and passlib.hash.sha256_crypt.verify(password, row[2]):
            db.close()
            return True
    db.close()
    return False
